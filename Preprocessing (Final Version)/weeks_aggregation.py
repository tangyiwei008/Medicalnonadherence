...
dfs = pd.read_csv('weeks_full.csv',
                  low_memory=False)
...
clab_member = pd.read_csv('MEMBER_enc.csv',
                          low_memory=False)
...
def calculate_list_counts(df, col_name):
    ...
    final_df.columns = ['_'.join(col).strip() if isinstance(col, tuple)
...
for shift in shifts:
    ...
    df_merged.to_csv('weeks_final.csv',
                     index=False)
