...
By Otto Gaulke
Description: recode claims procedures and diagnoses for less data sparsity.
'''
import pandas as pd
import numpy as np
import os
...
# load in procedure column
df_proc = pd.read_csv('rxclaims_full.csv',
                      usecols=['PROC_CD'],
                      low_memory=False)

# create an index change variable initialized at false
# this will keep track of indexes that have been changed or unchanged
# throughout the recoding process
index_change = df_proc.copy()
index_change['PROC_CD'] = False
index_change = index_change['PROC_CD']

# save indexes with values @NVL
index = df_proc['PROC_CD'] == '@NVL'
index_change |= index

# replace @NVL with NaN
...
# load PROCSRG1_CD
df_procsrg = pd.read_csv('rxclaims_full.csv',
                         usecols=['PROCSRG1_CD'],
                         low_memory=False)
...
# if PROCSRG1_CD exists, replace values in PROC_CD with PROCSRG1_CD values
...
for col in cols:
    ...
    df_diag_temp = pd.read_csv('rxclaims_full.csv',
                               usecols=[col],
                               low_memory=False)
...
# drop columns that will not be used or have been recoded
...
# add recoded columns to the rxclaims data
for col in cols:
