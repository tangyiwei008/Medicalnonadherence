...
print('Load')
# df = pd.read_csv('rxclaims_recoded.csv',encoding='unicode_escape',low_memory=False)
df = pd.read_csv('rxclaims_recoded_new.csv',encoding='unicode_escape',low_memory=False)

columns_to_drop = [
    'RX_line_id', 'COPAYDIFF_AMT', 'COPAYPEN_AMT',
...
members = df[df['Week'] == 49]['CLAB_ID'].unique()
df = df[df['CLAB_ID'].isin(members)]

df = df[df['Week'] < 49]

# Set Week_non_adhere to 53, remove later
...
for shift in shifts:
    ...
    df_merged = reduce(lambda left, right: pd.merge(left, right, on='CLAB_ID', how='outer'), dfs)
    ...
    df_merged.to_csv(f'rxclaims_new.csv', index=False)
