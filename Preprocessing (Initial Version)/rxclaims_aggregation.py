...
# Load the CLAB_MEMBER data
clab_member = pd.read_csv('CLAB_MEMBER.csv',
                          low_memory=False)

# Merge df with clab_member to get the Week_non_adhere column for each CLAB_ID
df = df.merge(clab_member[['CLAB_ID', 'Non_adhere', 'Week_non_adhere']],
              on='CLAB_ID',
              how='left')

# Randomizing "non-compliance" point for compliant members
# Get a Series of non-null values from the 'Week_non_adhere' column
non_null_weeks = df.groupby('CLAB_ID') \
                   .first()['Week_non_adhere'] \
                   .dropna() \
                   .astype(int).values

# Count the number of NaN values in the 'Week_non_adhere' column
nan_count = df['Week_non_adhere'] \
                .isnull() \
                .sum()

# Randomly sample from the non-null values to fill the NaNs
# Ensure replacement to handle cases where nan_count > len(non_null_weeks)
fill_values = np.random.choice(non_null_weeks,
                               size=nan_count,
                               replace=True)

# Fill the NaN values with the sampled data
df.loc[df['Week_non_adhere'].isnull(), 'Week_non_adhere'] = fill_values

# Convert the 'Week_non_adhere' column to integers
df['Week_non_adhere'] = df['Week_non_adhere'].astype(int)

# Convert days_from_index to weeks and adjust based on Week_non_adhere
...
def slice_data_dynamic(df_filtered, additional_shift, windows=[1, 5, 15, 25, 52]):
...
categorical_columns = [
    ...
    'COMPOUND_CD', 'PLANDRUGST_CD', 'DEA_CLASS_CD', 'ICD10_DIAGLN_CD',
    'DIAG1_CD', 'DIAG2_CD', 'DIAG3_CD', 'DIAG4_CD', 'DIAG5_CD', 'PROC_CD',
...
for shift in shifts:
    ...
    for i, old_df in enumerate(old_dfs):
        ...
        for col in categorical_columns:
            print(f'Generating {col} w{i}')
            counts_df = calculate_aggregate_counts(old_df,
                                                   col)
            if temp_df is None:
                temp_df = counts_df
            else:
    ...
    del df1r, df4r, df12r, df21r, df47r
    ...
    # # Add Week_non_adhere
    # df_merged = df_merged.sort_values(by='CLAB_ID', ascending=True)
    # add_col = df[['CLAB_ID','Non_adhere','Week_non_adhere']]
    # .sort_values(by='CLAB_ID', ascending=True)
    # [['Week_non_adhere','Non_adhere']]
    # df_merged[['Week_non_adhere', 'Non_adhere']] = add_col
    # df_merged['Non_adhere']

    # Fill all missing values in the merged df to 0
    df_merged.fillna(0,
                     inplace=True)

    print('Save' + str(shift))
    df_merged.to_csv(f'rxclaims_week_{shift+1}.csv',
                     index=False)
