...
dfs = [pd.read_csv(f'weeks_{n}..csv',
                   low_memory=False) for n in range(1, 11)]

df = pd.concat(dfs,
               ignore_index=True)

df.rename(columns={'Weekcount':'Week'},
          inplace=True)
...
nan_count = df['Week_non_adhere'].isnull().sum()
...
for shift in shifts:
    ...
    # First, rename columns in df1, df4, df12, df48 except for
    # the common columns
    ...
    df_merged.to_csv(f'ipro_week_{shift+1}.csv',
                     index=False)
