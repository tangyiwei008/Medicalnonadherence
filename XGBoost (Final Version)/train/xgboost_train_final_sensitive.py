...
def preprocess_data(data_filepath, labels_filepath, features_filepath):
    ...
    df = pd.merge(df, df2[['CLAB_ID', 'Non_adhere_last4', 'Non_adhere_flip']], on='CLAB_ID', how='left')
    ...
    X = df.drop(columns=['Non_adhere_last4', 'Non_adhere_flip'])
    X = X[top_features]
    ...
    y_flip_test = df.loc[X_test.index, 'Non_adhere_flip']

    # Apply SMOTE
    # smote = SMOTE(random_state=42)
    # X_train, y_train = smote.fit_resample(X_train, y_train)

    return X_train, X_test, y_train, y_test, race_test, y_flip_test


def train_and_evaluate(X, y, X_test, y_test, y_flip_test, scoring=weighted_scorer, i=0):
    ...
    with open(f'xgb_model_final_sensitive.pkl', 'wb') as file:
...
def plot_shap_waterfall(model, X, week, instance_index=0):
    ...
    # Save the figure
    plt.savefig(f'waterfall_final_sensitive.png', bbox_inches='tight')
...
def plot_combined_shap_summary(models, X_data_list, feature_names):
    ...
    plt.savefig('shap_summary_plot_final_sensitive.png', bbox_inches='tight')
    plt.show()
...
def display_top_10_percent_predictions(predictions, true_labels):
    ...
    positive_predictions_df = predictions_df
...
features_filepath = 'feature_importance_full_final_sensitive.csv'
...
for i, data_filepath in enumerate(filepaths, start=1):
    ...
    X_train, X_test, y_train, y_test, race_data, y_flip_test = preprocess_data(data_filepath, labels_filepath, features_filepath)
    ...
    optimized_model, y_pred_proba, accuracy, roc_auc, train_sizes, train_scores, test_scores = train_and_evaluate(X_train, y_train, X_test, y_test, y_flip_test, scoring='accuracy', i=i)
    ...
    importance_df.to_csv(f'feature_importance_final_sensitive.csv', index=False)
...
# Initialize a DataFrame to hold weekly importances
weekly_importances_df = pd.DataFrame(columns=['Feature'] + [f'Importance_w{i+1}' for i in range(num_weeks)])

# Populate the DataFrame with weekly importances
weekly_importances_df['Feature'] = feature_names
for i, (week, week_importance) in enumerate(weekly_feature_importance.items()):
    weekly_importances_df[f'Importance_w{i+1}'] = week_importance

# Calculate Mean, Geometric Mean, and Max importance
weekly_importances_df['Mean_Importance'] = weekly_importances_df.iloc[:, 1:num_weeks+1].mean(axis=1)
weekly_importances_df['GMean_Importance'] = weekly_importances_df.iloc[:, 1:num_weeks+1].apply(lambda x: gmean(x), axis=1)
weekly_importances_df['Max_Importance'] = weekly_importances_df.iloc[:, 1:num_weeks+1].max(axis=1)

weekly_importances_df.sort_values('Max_Importance', ascending=False, inplace=True)

# Save the DataFrame to a CSV file
# weekly_importances_df.to_csv('feature_importance_final.csv', index=False)
...
flip_mask = y_flip_test == 1
flip_accuracy = accuracy_score(y_test[flip_mask], final_binary_predictions[flip_mask])
flip_auc = roc_auc_score(y_test[flip_mask], y_pred_proba[flip_mask]) if np.any(flip_mask) else "No Flip Cases"

print('Results for those who remained adherent then went non-adherent:')
print(f"Flip Case Accuracy: {flip_accuracy}, Flip Case AUC: {flip_auc}")
print("Confusion Matrix:")
print(confusion_matrix(y_test[flip_mask], final_binary_predictions[flip_mask]))
print("Classification Report:")
print(classification_report(y_test[flip_mask], final_binary_predictions[flip_mask]))
