...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 14 12:48:19 2024
...
add_predictions_sensitive = pd.read_csv('/Users/emilyliu/Downloads/CAL/Rx Claim Data/predictions_sensitive.csv')
df = add_predictions_sensitive.merge(MAILORD, on='CLAB_ID', how='inner')
# Rename the merged column to 'PREDICTION'
df.rename(columns={'Max_Prediction': 'PREDICTION_SENSITIVE'}, inplace=True)
# Convert the values in the 'PREDICTION' column to percentages
df['PREDICTION_SENSITIVE'] = (df['PREDICTION_SENSITIVE'] * 100).apply(lambda x: "{:.0f}%".format(x))
...
df.to_csv('final_output_sensitive.csv', index=False)
